...
class BoardTestSuite {
    ...
    void testAddTaskListAverageWorkingOnTask(){
        ...
        double allTaskDays = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(tl -> tl.getTasks().stream())
                .map(t-> Double.valueOf(ChronoUnit.DAYS.between(t.getCreated(),LocalDate.now())))
                .reduce(0.0,(sum,current) -> sum = sum + current);
        double taskSum = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(tl -> tl.getTasks().stream())
                .count();
        double average = allTaskDays/taskSum;
        //Then
